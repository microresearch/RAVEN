...
void takens(int n,double v[],double dv[],double t) {
        ...
        double x,y,i1,i2,i3,w2,alfa=50,x2,y2, x3, y3, beta, cee;
	cee=0.0001f; beta=0.01f;
	
x=v[0]; y=v[1], i1=v[2], i2=v[3]; i3=v[4];x2=v[5],y2=v[6], w2=v[7];
x3=v[8]; y3=v[9];

 dv[0]=  y; //v1 and v0
 // dv[1]=  aa.alfa1*aa.gm*aa.gm + aa.beta1*aa.gm*aa.gm*(x) - aa.gm*aa.gm*x*x*x - aa.gm*x*x*y + aa.gm*aa.gm*x*x -aa.gm*x*y; // labia
 dv[1]=  (-aa.alfa1*(x)*aa.gm) + ((aa.beta1 - beta)*y*aa.gm) - (cee*x*x*y*aa.gm); // attempt at original 7.1 eqn

 dv[2]=i2; //v3 i3 is v4
 dv[3]=-aa.s1overLG*aa.s1overCH*i1 - aa.rdis*(aa.s1overLB+aa.s1overLG)*i2 + i3*(aa.s1overLG*aa.s1overCH - aa.rdis*aa.RB*aa.s1overLG*aa.s1overLB) + (aa.s1overLG)*aa.forcing2 + aa.rdis*aa.s1overLG*aa.s1overLB*aa.forcing1; // trachea and beak
 
 dv[4]=-(1/aa.s1overLG)*aa.s1overLB*i2 - aa.RB*aa.s1overLB*i3 + aa.s1overLB*aa.forcing1;  // beak? uses i2 from above - this is our output v[4]

 dv[5]= y2; // v6 x2 is v5
 dv[6]= aa.alfa2*aa.gm*aa.gm + aa.beta2*aa.gm*aa.gm*(x2) - aa.gm*aa.gm*x2*x2*x2 - aa.gm*x2*x2*y2 + aa.gm*aa.gm*x2*x2 - aa.gm*x2*y2; // 2nd labia
 dv[7]= 0.; 
 
 dv[8]=  y3;
 dv[9]=  -1.*aa.gamma2*aa.gamma2*aa.alfa3*x3 + aa.gamma2*aa.beta3*y3 - aa.gamma3*aa.gamma2*2.0*(40)*(40)*x3*x3*y3; // what is???
...
atenua=1;
 dt=1./(882000); // 44100 x20 steps
  ...
  // ancho is width largo is length finch is 3.5mm long
  // for raven length is 70mm and width 7mm so adjust these
  //  aa.Ancho1=0.2; aa.Ancho2=.2; aa.Ancho3=0.2;
    aa.Ancho1=2.; aa.Ancho2=2.; aa.Ancho3=3.;
  ancho1 = aa.Ancho1; ancho2 = aa.Ancho2; ancho3 = ancho2;
  //  largo1 = 1.5; largo2 =1.5;largo3 = 1.;
  largo1 = 30.; largo2 =30.;largo3 = 10.;
...
aa.s1overCH=(36*2.5*2/25.)*1e03;
aa.s1overLB=1.*1e-02;
 aa.s1overLG=(50*1e-01)/.5;
 aa.RB=1*1e07;
...
////////////////////////////////////////////
  ...
  while ((t<to)&&(v[1]>-5000000)){
    ...
    //    a[t]= (.50)* (1.01*(1.*(aa.A1*v[1]+aa.A2*v[6]+aa.A3*v[9]/10.)))+bb[t-tau1]; // original one
    a[t]= (.50)* (1.01*(1.*(aa.A1*v[1]+aa.A2*v[6]+aa.A3*v[9]/10.)))+bb[t-tau1]; // v[6] is second labia
    //    a[t]= (.50)* (1.01*(1.*(aa.A1*v[1]+aa.A3*v[9]/10.)))+bb[t-tau1]; // so this is all trachea stuff which goes into aa.forcing1 and 2
    //        a[t]= (.50)* (1.01*(1.*(aa.A1*v[1])))+bb[t-tau1]; // so this is all trachea stuff which goes into aa.forcing1 and 2
...
tiempot=tiempot+dt;
 ...
 aa.gm=12000.; /// lower is lower frequency
...
if(taux==20) {
  ...
  //		fscanf(ptr2,"%lg %lg",&nada,&aa.beta1);
		//		printf("%lg\n",aa.beta1);
  //  aa.beta1-=((rand()%10)/500000000.0f); // beta can go from 0 to minus3
  aa.beta1-=0.000000001f;		//		aa.beta1=-1.0f;
				//		fscanf(ptr3,"%lg %lg %lg",&nada,&nada,&aa.amplitud);
  aa.amplitud+=0.00001f;//((rand()%1024)/5000.0f); // should vary too
  fprintf(ptr4,"%lg\n",preout*10);
		//		fprintf(ptr4,"%lg\n",PRESSURE*10);
...
aa.beta1= aa.beta1+0.01*aa.noise;
 ...
 aa.s1overCH=(360/0.8)*1e01;  // what are these values about?
 aa.s1overLB=1.*1e-01; 
 aa.s1overLG=(1/12.); 
 aa.RB=(.5)*1e02; //beak out
