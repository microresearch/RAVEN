...
static inline void SVF_Process(SVF* svf, const float* in, float* out, u8 size, u8 mode) {
...
void runBANDStest_(float* incoming, float* outgoing, u8 band_size){
    ...
    u8 which=15;//(adc_buffer[SELX]>>8);// 4 bits to test
    mult_table[which]=1.0f;//(float)(adc_buffer[SELY]>>4)/4096.0f; //  12 bits 
